...
const EmployeeList: React.FC = () => {
  ...
  const [userRole, setUserRole] = useState<string | null>(null); // Tambahkan state untuk peran pengguna
    ...
    const fetchEmployees = async () => {
      try {
        ...
        if (!userResponse.ok || !["admin", "supervisor"].includes(userResult.role)) {
          throw new Error(
            userResult.message || "Akses ditolak: Hanya admin atau supervisor yang diizinkan"
          );
        }
        ...
        setErrorMessage(err instanceof Error ? err.message : "Gagal memuat data karyawan");
  ...
  const handleDelete = async () => {
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : "Gagal menghapus karyawan");
    }
  };
  ...
  const saveEditing = async (id: number) => {
    try {
      ...
      setErrorMessage(err instanceof Error ? err.message : "Gagal memperbarui data karyawan");
    }
  };
    ...
    <div className="space-y-4 max-w-5xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Add employee button - Hanya ditampilkan untuk admin */}
          {userRole === "admin" && (
            <Button
              variant="primary"
              ...
              data={paginatedData.map((item) => [
                ...
                editingId === item.id ? (
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-md max-w-[120px]"
                  />
                ) : (
                ...
                editingId === item.id ? (
                  <div className="flex items-center gap-2 min-w-[180px]">
                    <select
                      ...
                      onChange={(e) =>
                        handleEditChange("status", e.target.value as "aktif" | "nonaktif")
                      }
                    ...
                    <button className="p-1 text-red-600 hover:text-red-800" title="Batal" onClick={cancelEditing}>
